...
import { TBodegaG3 } from "../../../types/TypesBodega.types";
import { fetchBinBodega, fetchBodegasG3, fetchPDFBodegas } from "../../../redux/slices/bodegaSlice";
...
const TablaBodegaG3: FC<IBodegaG3Props> = ({ data, refresco, setRefresco }) => {
	...
	const exportToExcel = (data: Row<TBinBodega>[]) => {
		...
		XLSX.utils.book_append_sheet(wb, ws, 'Bodega G3')
		XLSX.writeFile(wb, 'bodega_g3.xlsx')
...
//     { id: 'codigo_tarja', header: '', className: 'w-32 text-center' },
//     { id: 'programa', header: '', className: 'w-56 text-center ' },
//     { id: 'fumigado', header: '', className: 'w-40 text-center ' },
//     { id: 'cc_tarja', header: '', className: 'w-40 text-center ' },
//     { id: 'kilos_fruta', header: '', className: 'w-48 text-center' },
//     { id: 'calibre', header: '', className: 'w-48 text-center' },
// 	{ id: 'variedad', header: '',className: 'w-48 text-center'},
// 	{ id: 'calidad', header: '',className: 'w-48 text-center'},
...
//   ]
	...
	// const pdf_bodegag3 = useAppSelector((state: RootState) => state.bodegas.pdf_bodegas)

	// useEffect(() => {
	// 	//@ts-ignore
	// 	dispatch(fetchPDFBodegas({ params: { search: 'g3'} ,token, verificar_token: verificarToken }))
	// }, [])


	return (
		<PageWrapper name='Lista Bodega G3' isProtectedRoute={true}>
			<Subheader>
				<SubheaderLeft>
					<FieldWrap
						...
						<CardHeaderChild>
							<div className="w-full flex gap-5">
								<Button
									...
									{/* <Link to='/pdf-bodegas' state={{ pdf: pdf_bodegag3, pathname: '/bodega-g3' }}>
										<Button
											variant="solid" 
