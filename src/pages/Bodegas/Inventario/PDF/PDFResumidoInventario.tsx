...
import { pdfResumidoInventario } from "../../../../redux/slices/bodegaSlice"
import { CALLES_BODEGA_G4 } from "../../../../utils/constante"


const PDFResumidoInventario = () => {
    const { id } = useParams()
    const pdf = useAppSelector((state: RootState) => state.bodegas.pdfResumidoInventario)
    const dispatch = useDispatch<ThunkDispatch<any, any, any>>()
    const token = useAppSelector((state: RootState) => state.auth.authTokens)
    const { verificarToken } = useAuth()
  
    useEffect(() => {
        dispatch(pdfResumidoInventario({ id_inventario: id, token, verificar_token: verificarToken }))
    }, [])  
    ...
    return (
        <PDFViewer style={{ height: '100%'}}>
            <Document title={`pdf_inventario_resumido_n_${pdf?.id}`}>
                    ...
                    <View style={{width: '95%', alignSelf: 'center'}}>
                        <View style={styles.header_superior}>
                            ...
                            <Text style={{ textAlign: 'center', fontSize: 14, alignSelf: 'center', width: '100%'}}>
                                {pdf?.tipo_inventario_label} N°{pdf?.id} Resumido
                            </Text>
                        ...
                        <View style={{
                            ...
                            <View style={{
                                ...
                                <View style={styles.header_date_info_box}>
                                    ...
                                    <Text style={styles.header_date_info_text}>{pdf && typeof(pdf.bins_no_validados) != 'string' ? pdf.bins_no_validados.length : pdf && typeof(pdf.bins_no_validados) == 'string' ? pdf?.bins_no_validados : null}</Text>
                                </View>
                            </View>
                        </View>

                        <Text style={{ textAlign: 'center', fontSize: 14, fontWeight: 'bold', marginTop: 10 }}>Detalle de Bins No Validados</Text>
                        <View style={{ 
                            width: '100%',
                            height: 30,
                            ...
                            <View style={styles.header_info_box_superior}>
                                <Text style={{ fontSize: 10, position: 'relative', top: -5}}>Calle</Text>
                            </View>

                            <View style={styles.header_info_box_superior}>
                                <Text style={{ fontSize: 10, position: 'relative', top: -5}}>Validado Por</Text>
                            </View>
                        ...
                        {
                            pdf && typeof(pdf.bins_no_validados) != 'string' && pdf.bins_no_validados.map(bin => {
                                return (
                                    <View style={{ 
                                        width: '100%',
                                        height: 30,
                                    ...
                                    >
                                        ...
                                        <View style={styles.header_info_box_superior}>
                                            <Text style={{ fontSize: 9}}>{bin.calle}</Text>
                                        </View>

                                        <View style={styles.header_info_box_superior}>
                                            <Text style={{ fontSize: 9}}>{bin.validado_por_label}</Text>
                                        </View>
...
export default PDFResumidoInventario