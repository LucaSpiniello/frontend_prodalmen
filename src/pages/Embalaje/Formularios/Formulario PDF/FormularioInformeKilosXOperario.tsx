...
const FormularioInformeKilosXOperario: FC<IInformeProduccion> = ({ setOpen }) => {
  ...
  useEffect(() => {
    dispatch(fetchOperariosFiltro({token, verificar_token: verificarToken, skill: '?skill=embalaje' }))
  }, []) 
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: any) => {
      try {
        ...
        const res = await fetchWithTokenPost(`api/embalaje/pdf_kilos_por_operario/`,
          {
            ...values,
          },
          token_verificado
        ...
        if (res.ok){
          ...
          const operario = operarios.find((operario: TOperarios) => operario.id === toInteger(values.operario))
          navigate('/emb/pdf-informe-kilos-x-operario/', { state: { programas_embalaje: data, desde: values.desde, hasta: values.hasta, pathname: pathname, operario: `${operario?.nombre} ${operario?.apellido}` }})
        }
      } catch (error) {
