...
const FormularioInformeEmbalaje: FC<IInformeProduccion> = ({ setOpen }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: any) => {
      try {
        ...
        const res = await fetchWithTokenPost(`api/embalaje/pdf_informe_embalaje/`,
          {
            ...values,
        ...
        if (res.ok){
          ...
          navigate('/emb/pdf-informe-embalaje/', { state: { pallets_embalaje: data, desde: state[0].startDate, hasta: state[0].endDate, pathname: pathname  }})
...
export default FormularioInformeEmbalaje
