...
import { fetchListaDetalleDiasOperarioEmbalaje } from "../../../../redux/slices/operarioSlice";

const columnHelperDias = createColumnHelper<TDetalleDiaOperario>();

function DetalleDiasOperarioEmbalaje({ datosOperario, open, setOpen } : {datosOperario: TOperarioEnEmbalaje, open: boolean, setOpen: Dispatch<SetStateAction<boolean>>}) {
    const { id } = useParams()
          ...
          <div className="font-bold">
            <Button color={props.row.original.ausente ? 'blue' : 'red'} variant='solid' icon={props.row.original.ausente ? `HeroPlus` : 'HeroMinus'} onClick={ async () => {
              try {
                ...
                const response = await fetchWithTokenPost(`api/embalaje/${id}/actualizar_ausente/`, {
                  dia_id: props.row.original.id,
                  ausente: !props.row.original.ausente
                }, token_verificado)
                if (response.ok) {
                  dispatch(fetchListaDetalleDiasOperarioEmbalaje({token, id_operario: datosOperario?.operario, id_programa: datosOperario?.programa, verificar_token: verificarToken}))
...
export default DetalleDiasOperarioEmbalaje
