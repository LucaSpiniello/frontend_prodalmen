...
import { fetchPDFSalidaEmbalaje } from '../../../redux/slices/embalajeSlice';
import { PDFPalletSalidaEmbalaje } from '../../../types/TypesEmbalaje.type';
...
const PDFDocumentoEntradaSalida = () => {
  ...
  const perfilData = useAppSelector((state: RootState) => state.auth.dataUser)
	const pdf_salida = useAppSelector((state: RootState) => state.embalaje.pdf_salida_embalaje)
  const dispatch = useDispatch<ThunkDispatch<any, any, any>>()
  const token = useAppSelector((state: RootState) => state.auth.authTokens)
  const { verificarToken } = useAuth()

  useEffect(() => {
    dispatch(fetchPDFSalidaEmbalaje({ id: parseInt(id!), token, verificar_token: verificarToken }))
  }, [])
      ...
      <Document title={`Documento Salida Programa Embalaje N° ${id}`}>
        <Page size={'LEGAL'} orientation='landscape'>
         <View style={styles.header}>
          ...
          <View style={{
            ...
            <View style={{
              ...
              <View style={styles.header_date_info_box}>
                ...
                <Text style={styles.header_date_info_text}>{(pdf_salida?.programa.kilos_ingresados ?? 0).toLocaleString()} kgs</Text>
              </View>

              <View style={styles.header_date_info_box}>
                <Text style={styles.header_date_info_text}>Total Kilos Resultante: </Text>
                <Text style={styles.header_date_info_text}>{(pdf_salida?.programa.kilos_resultantes ?? 0).toLocaleString()} kgs</Text>
              </View>

              <View style={styles.header_date_info_box}>
                <Text style={styles.header_date_info_text}>Observaciones: </Text>
                <Text style={styles.header_date_info_text}>{(pdf_salida?.programa.observaciones ?? 0).toLocaleString()} </Text>
              </View>
            ...
            <View style={{
              ...
              <View style={styles.header_date_info_box}>
                ...
                <Text style={styles.header_date_info_text}>{format(pdf_salida?.programa.fecha_inicio_embalaje!, { date: 'full', time: 'short' }, 'es' )} </Text>
              </View>
          ...
          {
            pdf_salida?.pallets.map((entrada: PDFPalletSalidaEmbalaje) => {
              return (
                <View style={{ 
                  width: '100%',
                  height: 30,
                  ...
                  <View style={styles.header_info_box_superior}>
                   <Text style={{ fontSize: 9}}>{format(entrada.fecha_creacion, { date: 'medium', time: 'short'}, 'es' )}</Text>
                  </View>
