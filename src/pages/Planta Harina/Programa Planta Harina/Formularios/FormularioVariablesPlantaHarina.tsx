...
const FormularioVariablesPlantaHarina = ({ setOpen }: { setOpen: Dispatch<SetStateAction<boolean>>}) => {
  const variable_planta_harina = useAppSelector((state: RootState) => state.planta_harina.variable_planta_harina)
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(actualizar_variables_planta_harina({ id: parseInt(id!), token, data: {...values}, verificar_token: verificarToken, action: setOpen }))
    }
  })

  useEffect(() => {
    if (variable_planta_harina){
      formik.setFieldValue('lectura_gas_inicio', variable_planta_harina.lectura_gas_inicio)
      formik.setFieldValue('lectura_luz_inicio', variable_planta_harina.lectura_luz_inicio)
      formik.setFieldValue('lectura_gas_termino', variable_planta_harina.lectura_gas_termino)
      formik.setFieldValue('lectura_luz_termino', variable_planta_harina.lectura_luz_termino)
    }
  }, [variable_planta_harina])
  ...
  return (
    <Container>
      <Card>
        <CardBody>
          {
            programa_planta_harina?.estado_programa !== '5' && !(variable_planta_harina?.lectura_gas_inicio && variable_planta_harina.lectura_luz_inicio)
              ? (
              ...
              : !(variable_planta_harina?.lectura_gas_termino && variable_planta_harina.lectura_luz_termino)
                ?
                  (
                  <section className='flex w-full gap-5'>
          ...
          <div className='flex justify-end mt-5'>
            <Button
              ...
              >
              {programa_planta_harina?.estado_programa !== '5' && !(variable_planta_harina?.lectura_gas_inicio && variable_planta_harina.lectura_luz_inicio)  ? 'Agregar variables inicio' : 'Agregar variables Termino'}
            </Button>
          </div>
...
export default FormularioVariablesPlantaHarina
