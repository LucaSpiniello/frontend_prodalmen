...
import { LIMPIAR_BINS_PARA_PLANTA_HARINA, QUITAR_BIN_PARA_PLANTA_HARINA } from '../../../../../redux/slices/plantaHarinaSlice';


const TablaBinPlantaHarina = ({refresh, setRefresh}: {refresh: boolean, setRefresh: Dispatch<SetStateAction<boolean>>}) => {
  const { id } = useParams()
            ...
            <div className='w-full flex justify-center gap-5 flex-wrap md:flex-wrap'>
                ...
                <Tooltip text='Quitar Bin'>
                  <Button
                    variant='solid'
                    onClick={() => {
                      dispatch(QUITAR_BIN_PARA_PLANTA_HARINA(id))
  ...
  const agregarBinASeleccion = async () => {
    ...
    const response = await fetchWithTokenPost(`api/programas/${id}/bines-para-programa/registrar_bins/`, 
      {"bins": bins_para_planta_harina}, token_verificado)
    if (response.ok){
      toast.success('Bins fueron ingresados exitosamente!!')
      //@ts-ignore
      dispatch(fetchBinsPepaCalibrada({ token, verificar_token: verificarToken }))
      navigate(`/ph/ph-prog/${id}/`, { replace: true })
    } else {
  ...
  return (
    <Container breakpoint={null} className='w-full overflow-auto p-0'>
          ...
          <CardHeader>
            ...
            {
              bins_para_planta_harina.length >= 1
                ? (
                  <ModalForm
                    open={openButton}
                    setOpen={setOpenButton}
                    textTool='Agregar a Programa Planta Harina'
                    variant="solid"
                    color='blue'
                    colorIntensity='700'
                    width={`w-4/12 hover:scale-105`}
...
export default TablaBinPlantaHarina;
