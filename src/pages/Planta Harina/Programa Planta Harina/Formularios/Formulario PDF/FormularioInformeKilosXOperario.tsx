...
const FormularioInformeKilosXOperario: FC<IInformeProduccion> = ({ setOpen }) => {
  ...
  useEffect(() => {
    dispatch(fetchOperariosFiltro({token, verificar_token: verificarToken, skill: '?skill=p_harina' }))
  }, []) 
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: any) => {
      try {
        ...
        const res = await fetchWithTokenPost(`api/programas/pdf_kilos_por_operario/`,
          {
            ...values,
          },
          token_verificado
        ...
        if (res.ok){
          ...
          navigate('/ph/ph-prog/pdf-informe-kilos-x-operario', { state: { programas: data, desde: values.desde, hasta: values.hasta, pathname: pathname, operario: `${operario?.nombre} ${operario?.apellido}` }})
        }
      } catch (error) {
