...
const FormularioTipoRechazoPlantaHarina = ({ setOpen}: {setOpen: Dispatch<SetStateAction<boolean>>}) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(registro_tipo_rechazo_planta_harina({ id: parseInt(id!), data: { registrado_por: perfil?.id, programa: id, ...values}, token, verificar_token: verificarToken, action: setOpen }))
    }
  })
      ...
      <Card>
        <CardBody>
          ...
          <div className='w-full'>
            ...
            <div className='w-full'>
              <Label htmlFor='kilos_rechazo'>Kilos: </Label>
              <Validation
                isValid={formik.isValid}
                isTouched={formik.touched.kilos_rechazo ? true : undefined}
                invalidFeedback={formik.errors.kilos_rechazo ? String(formik.errors.kilos_rechazo) : undefined}>
                <FieldWrap>
                  <Input
                    type='number'
                    name='kilos_rechazo'
                    ...
                    className='py-3'
                    value={formik.values.kilos_rechazo}
                  />
                </FieldWrap>
              </Validation>
            </div>
