...
import { AGREGAR_BIN_PARA_PROCESO_PLANTA_HARINA, LIMPIAR_BINS_PARA_PROCESO_PLANTA_HARINA } from '../../../../../redux/slices/procesoPlantaHarina';
...
const TablaBinBodegaProcesoPlantaHarina = ({refresh} : {refresh:boolean}) => {
  ...
  useEffect(() => {
    ...
    if (bin_bodega && bin_bodega.length > 0) {
      bin_bodega.forEach((element) => {
        if (bins_para_proceso_planta_harina.find(bin => bin.id === element.id)) {
        } else {
          lista.push(element)
        }
      })
  ...
  // useEffect(() => {
  //   dispatch(fetchBinBodega({ params: { search: !filtroBodega ? 'g4,g5,g6&programa=planta-harina' : filtroBodega }, token, verificar_token: verificarToken }))
  // }, [filtroBodega])

  useEffect(() => {
    // dispatch(fetchBinBodega({ params: { search: !filtroBodega ? 'g1,g2' : filtroBodega }, token, verificar_token: verificarToken }));
    dispatch(listaBinBodegaFiltroThunk({token: token, verificar_token: verificarToken, filtro: !filtroBodega ? 'g4,g5,g6' : filtroBodega}))
                ...
                <Tooltip text='Agregar A Planta Harina'>
                  <Button
                    ...
                    onClick={() => {
                      ...
                      dispatch(AGREGAR_BIN_PARA_PROCESO_PLANTA_HARINA(info.row.original))
                    }}
...
export default TablaBinBodegaProcesoPlantaHarina;
