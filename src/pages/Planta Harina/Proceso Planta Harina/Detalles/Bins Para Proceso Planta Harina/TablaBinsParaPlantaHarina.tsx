...
import { TBinParaProcesoPlantaHarina } from '../../../../../types/typesPlantaHarina';
import {eliminar_bin_para_planta_harina, procesado_masivo_bin_para_planta_harina } from '../../../../../redux/slices/plantaHarinaSlice';
import { CheckboxChangeEvent } from 'antd/es/checkbox';
import Checkbox from '../../../../../components/form/Checkbox';
import { GiCheckedShield } from 'react-icons/gi';
import { ImRadioChecked } from 'react-icons/im';
import { actualizar_estado_bin_para_proceso_planta_harina, eliminar_bin_para_proceso_planta_harina } from '../../../../../redux/slices/procesoPlantaHarina';
...
const TablaBinsParaProcesoPlantaHarina: FC<IProduccionProps> = () => {
  const { id } = useParams()
  ...
  const columnHelper = createColumnHelper<TBinParaProcesoPlantaHarina>()
  
  const [rowSelection, setRowSelection] = useState({});
  const [lotesParaMasivo, setLotesParaMasivo] = useState<TBinParaProcesoPlantaHarina[]>([])


  useEffect(() => {
    const selectedKeys = Object.keys(rowSelection);
    const selectedIndexes = selectedKeys.map(key => parseInt(key));
    const bin_procesados = bins_para_proceso_planta_harina.filter(bin => bin.procesado !== true)
    const lotes_encontrados = bins_para_proceso_planta_harina.filter((_item, index) => selectedIndexes.includes(index))
  ...
  const columns = [
      ...
      ? (
        columnHelper.display({
          ...
          cell: (info) => (
            <div>
                {
                  !info.row.original.procesado && !(proceso_planta_harina?.estado_proceso === '5' || proceso_planta_harina?.estado_proceso !== '2')
                    ? (
    ...
    columnHelper.display({
        ...
        return (
                ...
                ?
                  (
                    <Button
                      ...
                      colorIntensity='600'
                      onClick={() => {
                        dispatch(actualizar_estado_bin_para_proceso_planta_harina({ id: parseInt(id!), params: { id_bin: info.row.original.id }, data: { procesado: true }, token, verificar_token: verificarToken }))
                      }}
                      className='w-20 rounded-md h-12 text-white flex items-center justify-center p-2 hover:scale-105'
                      >
          ...
          {
            estado || (proceso_planta_harina?.estado_proceso! === '0' || proceso_planta_harina?.estado_proceso! !== '2' && proceso_planta_harina?.estado_proceso! <= '3')
              ? null
              : (
                <Button
                  ...
                  color='red'
                  colorIntensity='700'
                  onClick={() => dispatch(eliminar_bin_para_proceso_planta_harina({ id: parseInt(id!), params: { id_bin: info.row.original.id }, token, verificar_token: verificarToken  }))}
                  className='w-20 rounded-md h-12 text-white flex items-center justify-center p-2 hover:scale-105'
                  >
  ...
  const table = useReactTable({
		data: bins_para_proceso_planta_harina ? bins_para_proceso_planta_harina : [],
		columns,
		state: {
			sorting,
			globalFilter,
  ...
  return (
    <Container breakpoint={null} className='w-full overflow-auto'>
          ...
          <CardHeader>
            ...
            {
              !bins_para_proceso_planta_harina.every(bin => bin.procesado === true) && lotesParaMasivo.length > 1
                ? (
                  <Button
                    variant='solid'
...
export default TablaBinsParaProcesoPlantaHarina;
