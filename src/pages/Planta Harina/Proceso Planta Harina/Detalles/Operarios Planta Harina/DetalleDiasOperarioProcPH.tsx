...
import { TDetalleDiaOperario, TListaOperarioEnProduccion } from "../../../../../types/TypesProduccion.types";
import { useAuth } from "../../../../../context/authContext";
import { fetchListaDetalleDiasOperarioProcPH, fetchListaDetalleDiasOperarioProduccion, fetchListaDetalleDiasOperarioProgPH } from "../../../../../redux/slices/operarioSlice";
...
function DetalleDiasOperarioProcPH({ datosOperario, open, setOpen } : {datosOperario: TOperarioProcesoPlantaHarina, open: boolean, setOpen: Dispatch<SetStateAction<boolean>>}) {
    const { id } = useParams()
          ...
          <div className="font-bold">
            <Button color={props.row.original.ausente ? 'blue' : 'red'} variant='solid' icon={props.row.original.ausente ? `HeroPlus` : 'HeroMinus'} onClick={ async () => {
              try {
                ...
                const response = await fetchWithTokenPost(`api/procesos/${id}/actualizar_ausente/`, {
                  dia_id: props.row.original.id,
                  ausente: !props.row.original.ausente
                }, token_verificado)
                if (response.ok) {
                  dispatch(fetchListaDetalleDiasOperarioProcPH({token, id_operario: datosOperario?.operario, id_programa: datosOperario?.programa, verificar_token: verificarToken}))
...
export default DetalleDiasOperarioProcPH
