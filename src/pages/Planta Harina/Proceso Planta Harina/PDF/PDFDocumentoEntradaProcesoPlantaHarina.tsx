...
import { fetchPDFEntradaPlantaHarina } from '../../../../redux/slices/plantaHarinaSlice';
import { fetchPDFEntradaProcesoPlantaHarina } from '../../../../redux/slices/procesoPlantaHarina';
...
const PDFDocumentoEntradaProcesoPlantaHarina = () => {
  const { id } = useParams()
  // const { state } = useLocation()
  const perfilData = useAppSelector((state: RootState) => state.auth.dataUser)

  const programa = useAppSelector((state: RootState) => state.proceso_planta_harina.proceso_planta_harina)
  const dispatch = useDispatch<ThunkDispatch<any, any, any>>()
  const token = useAppSelector((state: RootState) => state.auth.authTokens)
  const { verificarToken } = useAuth()

  const pdf = useAppSelector((state: RootState) => state.proceso_planta_harina.pdf_documento_entrada)

  useEffect(() => {
    dispatch(fetchPDFEntradaProcesoPlantaHarina({ id: parseInt(id!), token, verificar_token: verificarToken }))
  }, [])
  ...
  return (
    <PDFViewer style={{ height: '100%'}}>
      <Document title={`Documento Entrada Proceso Planta Harina N° ${id}`}>
        <Page>
...
export default PDFDocumentoEntradaProcesoPlantaHarina