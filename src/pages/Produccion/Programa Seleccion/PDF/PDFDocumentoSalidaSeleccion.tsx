...
const PDFDocumentoSalida = () => {
  ...
  const perfilData = useAppSelector((state: RootState) => state.auth.dataUser)
	const pdf_salida = useAppSelector((state: RootState) => state.seleccion.pdf_detalle_salida)
  const dispatch = useDispatch<ThunkDispatch<any, any, any>>()
  const token = useAppSelector((state: RootState) => state.auth.authTokens)
  const { verificarToken } = useAuth()

  useEffect(() => {
    dispatch(fetchPDFSalidaSeleccion({ id: parseInt(id!), token, verificar_token: verificarToken }))
  }, [])

  return (
    <PDFViewer style={{ height: '100%'}}>
      <Document title='Documento Salida Selección'>
        ...
        <View style={styles.header}>
          <View style={styles.header_superior}>
            ...
            <Text style={{ width: 240, textAlign: 'center', fontSize: 14, position: 'relative', left: 10, top: 10}}>
              Documento Salida al Programa de Selección N° {id}
            </Text>
          ...
          <View style={{
            ...
            <View style={{
              ...
              <View style={styles.header_date_info_box}>
                ...
                <Text style={styles.header_date_info_text}>{(pdf_salida?.bines.reduce((acc, entrada) => entrada.kilos + acc, 0) ?? 0).toLocaleString()}</Text>
              </View>
...

          {
           ...
           <View style={{ 
            ...
            <View style={styles.header_info_box_superior}>
             <Text style={{ fontSize: 10, position: 'relative', top: -5}}>Tipo SubProducto</Text>
            </View>
