...
const TablaDetalleDarioOperario = ({ rut }: { rut: string | undefined } ) => {
  ...
  const operario = useAppSelector((state: RootState) => state.seleccion.operario_seleccion_individual)
    ...
    try {
      ...
      const response = await fetchWithTokenDeleteAction(`api/seleccion/${id_programa}/operarios/eliminar_registro_dia_por_rut_y_id/`, 
      {
        rut: rut,
				id: id
      }
      ...
      if (response.ok){
        ...
        dispatch(fetchOperariosEnSeleccion({ id: parseInt(id_programa!), params: { rut: rut }, token, verificar_token: verificarToken }))
				dispatch(fetchOperariosPorDiaEnSeleccion({ id: parseInt(id_programa!), token, verificar_token: verificarToken }))
  ...
  const columnHelper = createColumnHelper<TOperarioSeleccion>();
  const columns = [
