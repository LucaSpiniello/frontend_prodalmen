...
import { fetchListaDetalleDiasOperarioProduccion, fetchListaDetalleDiasOperarioReproceso, fetchListaDetalleDiasOperarioSeleccion } from "../../../../../redux/slices/operarioSlice";
...
import { TOperarioEnSeleccion } from "../../../../../types/TypesSeleccion.type";

const columnHelperDias = createColumnHelper<TDetalleDiaOperario>();

function DetalleDiasOperarioReproceso({ datosOperario, open, setOpen } : {datosOperario: TOperarioEnSeleccion, open: boolean, setOpen: Dispatch<SetStateAction<boolean>>}) {
    const { id } = useParams()
          ...
          <div className="font-bold">
            <Button color={props.row.original.ausente ? 'blue' : 'red'} variant='solid' icon={props.row.original.ausente ? `HeroPlus` : 'HeroMinus'} onClick={ async () => {
              try {
                ...
                const response = await fetchWithTokenPost(`api/seleccion/${id}/actualizar_ausente/`, {
                  dia_id: props.row.original.id,
                  ausente: !props.row.original.ausente
                }, token_verificado)
                if (response.ok) {
                  dispatch(fetchListaDetalleDiasOperarioSeleccion({token, id_operario: datosOperario?.operario, id_programa: datosOperario?.seleccion, verificar_token: verificarToken}))
