...
import { fetchProgramaReproceso, fetchTarjasResultantesReproceso } from "../../../../redux/slices/reprocesoSlice"
import { fetchCalibracionTarjaReprocesoIndividual } from "../../../../redux/slices/controlcalidadSlice"
...
const FormularioControlCalidadTarjaReproceso : FC<IFormCC> = ({ id_lote, isOpen, tipo_resultante, refresh, setRefresh }) => {
  const { id } = useParams()
  ...
  const actualizarEstadoTarja = async (id_lote: number) => {
    ...
    const res = await fetchWithTokenPatch(`api/reproceso/${id}/tarjas_resultantes/${id_lote}/`,
      {
        cc_tarja: true,
        reproceso: id
      },
  ...
  const control_calidad_tarja_reproceso = useAppSelector((state: RootState) => state.control_calidad.cc_calibracion_tarja_reproceso_individual)

  useEffect(() => {
    //@ts-ignore
    dispatch(fetchCalibracionTarjaReprocesoIndividual({ id: id_lote, token, verificar_token: verificarToken }))
  }, [])
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: any) => {
      try {
        const token_verificado = await verificarToken(token!)
        if (!token_verificado) throw new Error('Token no verificado')
        const res = await fetchWithTokenPatch(`api/reproceso/cdc-tarjaresultante/${id_lote}/`, 
          {
            ...values,
            estado_cc: '3'
          },
  ...
  const calcularPepaSana = (): number => {
    ...
    const result = (
      cantidad_muestra - trozo - picada - hongo - daño_insecto - dobles - goma - basura - mezcla_variedad - fuera_color - punto_goma - vana_deshidratada - canuto
    );
  ...
  useEffect(() => {
    if (control_calidad_tarja_reproceso){
      formik.setValues({
        ...control_calidad_tarja_reproceso
      })
    }
  }, [control_calidad_tarja_reproceso])
          ...
          tipo_resultante === 'Borrel'
            ? (
              <div className='md:row-start-6 md:col-span-4'>
                ...
                <Validation
                  isValid={formik.isValid}
                  isTouched={formik.touched.canuto ? true : undefined}
                  invalidFeedback={formik.errors.canuto ? String(formik.errors.canuto) : undefined}>
                  <FieldWrap>
                    <Input
                      type='number'
                      name='canuto'
...
export default FormularioControlCalidadTarjaReproceso
