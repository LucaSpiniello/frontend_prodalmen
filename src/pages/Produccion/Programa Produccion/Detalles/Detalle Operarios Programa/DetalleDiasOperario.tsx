...
import { fetchListaDetalleDiasOperarioProduccion } from "../../../../../redux/slices/operarioSlice";
...
function DetalleDiasOperario({ datosOperario, open, setOpen } : {datosOperario: TListaOperarioEnProduccion, open: boolean, setOpen: Dispatch<SetStateAction<boolean>>}) {
    const { id } = useParams()
          ...
          <div className="font-bold">
            <Button color={props.row.original.ausente ? 'blue' : 'red'} variant='solid' icon={props.row.original.ausente ? `HeroPlus` : 'HeroMinus'} onClick={ async () => {
              try {
                ...
                const response = await fetchWithTokenPost(`api/produccion/${id}/actualizar_ausente/`, {
                  dia_id: props.row.original.id,
                  ausente: !props.row.original.ausente
                }, token_verificado)
                if (response.ok) {
                  dispatch(fetchListaDetalleDiasOperarioProduccion({token, id_operario: datosOperario?.operario, id_programa: datosOperario?.produccion, verificar_token: verificarToken}))
...
export default DetalleDiasOperario
