...
const FormularioControlCalidadTarja : FC<IFormCC> = ({ id_lote, isOpen, tipo_resultante, refresh, setRefresh }) => {
  ...
  const control_calidad_tarja = useAppSelector((state: RootState) => state.control_calidad.cc_calibracion_tarja_produccion_individual)

  useEffect(() => {
    //@ts-ignore
    dispatch(fetchCalibracionTarjasResultantesProduccionIndividual({ id: id_lote, token, verificar_token: verificarToken }))
  }, [])
  ...
  const actualizarEstadoTarja = async (id_lote: number) => {
    ...
    const res = await fetchWithTokenPatch(`api/produccion/${id}/tarjas_resultantes/${id_lote}/`,
      {
        cc_tarja: true,
        produccion: id
      },
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: any) => {
      try {
        ...
        const res = await fetchWithTokenPatch(`api/produccion/cdc-tarjaresultante/${id_lote}/`, 
          {
            ...values,
  ...
  useEffect(() => {
    if (control_calidad_tarja){
      formik.setFieldValue('variedad', control_calidad_tarja.variedad)
    }
  }, [control_calidad_tarja])
