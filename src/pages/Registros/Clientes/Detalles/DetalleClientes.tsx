...
import { fetchWithTokenPatch, fetchWithTokenPost, fetchWithTokenPut } from '../../../../utils/peticiones.utils'
import { fetchClienteSeleccionado, fetchClientes } from '../../../../redux/slices/clientes'
import ButtonsClientes, { OPTIONS_CLIENTES, TTabsClientes } from '../Botones/ButtonsClientes'
import DetalleSucursales from './DetalleSucursales'
import { ThunkDispatch } from '@reduxjs/toolkit';
import { useDispatch } from 'react-redux';
import DetalleCuentasCorrientes from './DetalleCuentasCorrientes'
import DetalleRepresentantes from './DetalleRepresentantes'
...
const DetalleClientes: FC<IFormComercializadorProps> = ({ setOpen, tipo_cliente, rut }) => {
  ...
  const cliente = useAppSelector((state: RootState) => state.clientes.cliente_seleccionado)
	const [activeTab, setActiveTab] = useState<TTabsClientes>(OPTIONS_CLIENTES.CT)
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (tipo_cliente === 'clientemercadointerno'){
        const res = await fetchWithTokenPut(`api/clientes_mercado_interno/${rut}/`, {
        ...
        const res_v = await fetchWithTokenPut(`api/clientes_exportacion/${rut}/`, {
  ...
  useEffect(() => {
    if (rut){
      dispatch(fetchClienteSeleccionado({  params: { rut: rut }, token, verificar_token: verificarToken }))
    }
  }, [rut])

  useEffect(() => {
    if (tipo_cliente === 'clientemercadointerno' && cliente){
      //@ts-ignore
      formik.setValues({
        ...
        ciudad: cliente.ciudad ?? 0,
      })
    } else if (tipo_cliente === 'clienteexportacion'){
      formik.setFieldValue('dni_cliente', cliente?.dni_cliente)
      formik.setFieldValue('razon_social', cliente?.razon_social)
      formik.setFieldValue('telefono', cliente?.telefono)
      formik.setFieldValue('pais', cliente?.pais)
      formik.setFieldValue('ciudad', cliente?.ciudad)
      formik.setFieldValue('direccion_1', cliente?.direccion_1)
      formik.setFieldValue('direccion_2', cliente?.direccion_2)
      formik.setFieldValue('codigo_postal', cliente?.ciudad)
      formik.setFieldValue('email_cliente', cliente?.email_cliente)
      formik.setFieldValue('pagina_web', cliente?.pagina_web)
  ...
  return (
    <Container breakpoint={null} className='!p-0'>
      <div className='flex  mb-2'>
        <ButtonsClientes activeTab={activeTab} setActiveTab={setActiveTab} tipo_cliente={tipo_cliente}/>
      </div>
      {
        activeTab.text === 'Clientes'
          ? (
            <Card>
                            ...
                            <Validation
                              ...
                              <FieldWrap>
                                <SelectReact
                                  ...
                                  name='region'
                                  className='h-12'
                                  value={optionsRegion.find(option => option?.value === String(formik.values.region))}
                                  onChange={(value: any) => {
                            ...
                            <Validation
                              ...
                              <FieldWrap>
                                <SelectReact
                                  ...
                                  name='provincia'
                                  className='h-12'
                                  value={optionsProvincia.find(option => option?.value === String(formik.values.provincia))}
                                  onChange={(value: any) => {
                            ...
                            <Validation
                              ...
                              <FieldWrap>
                                <SelectReact
                                  ...
                                  name='comuna'
                                  className='h-12'
                                  value={optionsComunas.find(option => option?.value === String(formik.values.comuna))}
                                  onChange={(value: any) => {
                              ...
                              <Validation
                                ...
                                <FieldWrap>
                                  <SelectReact
                                    ...
                                    name='pais'
                                    className='h-12'
                                    value={optionsPaises.find(option => option?.value === String(formik.values.pais))}
                              ...
                              <Validation
                                ...
                                <FieldWrap>
                                  <SelectReact
                                    ...
                                    name='ciudad'
                                    className='h-12'
                                    value={optionsCiudades.find(option => option?.value === String(formik.values.ciudad))}
                                    onChange={(value: any) => {
          ...
          : activeTab.text === 'Sucursales'
            ? <DetalleSucursales rut={rut} tipo_cliente={tipo_cliente}/>  
            : activeTab.text === 'Cuentas Corrientes'
              ? <DetalleCuentasCorrientes />
              : activeTab.text === 'Representantes'
                ? <DetalleRepresentantes />
                : null
      }
