...
const DetalleCCTarjaReproceso = () => {
  const { id } = useParams()
  const control_calidad = useAppSelector((state: RootState) => state.control_calidad.cc_calibracion_tarja_reproceso_individual)
  ...
  useEffect(() => {
    if (id){
      dispatch(fetchCalibracionTarjaReprocesoIndividual({ id: parseInt(id), token, verificar_token: verificarToken }))
    }
  }, [id])
  ...
  return (
   <Container>
    <Card>
      <CardHeader>
        <CardTitle>Detalle de Control de Calidad Tarja Reproceso Resultante N° {control_calidad?.codigo_tarja}</CardTitle>
      </CardHeader>
      <CardBody>
        <article className={`row-start-2 col-span-3 dark:bg-zinc-800 bg-zinc-100 w-full h-full flex md:flex-row lg:flex-row justify-between items-center gap-x-10 mx-auto`}>
          <div className='w-full md:5/12 lg:5/12 justify-between lg:h-20 flex flex-col lg:flex-row rounded-md lg:gap-x-4 gap-y-4 p-2'>
            <div className={`border dark:border-zinc-700 w-full rounded-md h-full flex flex-col justify-center px-2`}>
        ...
        <article className={`row-start-4 row-span-4 col-span-3 w-full h-full dark:bg-zinc-800 bg-zinc-100 flex flex-col lg:flex-col justify-between `}>
