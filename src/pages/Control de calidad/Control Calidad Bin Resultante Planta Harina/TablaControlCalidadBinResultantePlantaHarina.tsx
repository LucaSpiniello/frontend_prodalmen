...
import { TControlCalidadBinResultantePlantaHarina } from '../../../types/typesPlantaHarina';
...
import DetalleBinResultantePlantaHarina from '../../Planta Harina/Programa Planta Harina/Detalles/Bins Resultantes Planta Harina/DetalleBinResultantePlantaHarina';
import DetalleControlCalidadBinResultantePlantaHarina from './DetalleControlCalidadBinResultantePlantaHarina';

const TablaControlCalidadBinResultantePlantaHarina = () => {
  ...
  const cc_bin_resultante_planta_harina = useAppSelector((state: RootState) => state.planta_harina.controles_calidad_bin_resultante_planta_harina)
	const dispatch = useAppDispatch()
  const token = useAppSelector((state: RootState) => state.auth.authTokens)
  const { verificarToken } = useAuth()

  useEffect(() => {
    dispatch(fetchCCBinesResultantePlantaHarina({ token, verificar_token: verificarToken }))
  }, [])
  ...
  const columnHelper = createColumnHelper<TControlCalidadBinResultantePlantaHarina>();
	...
	const columns = [
		...
		columnHelper.display({
			id: 'calidad',
			cell: (info) => {
				return <div className='font-bold text-center'>{info.row.original.calidad_label}</div>;
			},
			header: 'Calidad',
		}),
		columnHelper.display({
		...
		columnHelper.display({
            ...
            <ModalForm
              ...
              >
                <DetalleControlCalidadBinResultantePlantaHarina id_bin={info.row.original.bin_resultante}/>
            </ModalForm>

						{
							hasGroup(['controlcalidad']) && info.row.original.estado_cc !== '1'
								? (
									<ModalForm
	...
	const table = useReactTable({
		data: cc_bin_resultante_planta_harina,
		columns,
		state: {
			sorting,
			globalFilter,
	...
	return (
    <PageWrapper name='Lista Control Calidad Bin Planta Harina'>
			<Subheader>
				<SubheaderLeft>
					<FieldWrap
			...
			<Container breakpoint={null} >
				<Card className='h-full'>
					<CardHeader>
						<CardHeaderChild>
							<CardTitle>Controles de Calidad Bin Resultante Planta Harina</CardTitle>
							<Badge
								variant='outline'
...
export default TablaControlCalidadBinResultantePlantaHarina
