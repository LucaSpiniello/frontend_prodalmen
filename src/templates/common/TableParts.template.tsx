...
export interface TableColumn {
	...
	header?: string;
	className?: string;
	// Otras propiedades opcionales de la columna...
}
...
export const TableHeaderTemplate: FC<ITableHeaderTemplateProps> = ({ table, columnas }) => {
	return (
		<THead>
			{table.getHeaderGroups().map((headerGroup) => (
				<Tr key={headerGroup.id}>
					{headerGroup.headers.map((header) => {
						const columnConfig = columnas && columnas.find(col => col.id === header.column.id);
						const columnClassName = columnConfig && columnConfig.className
			...
			{table.getRowModel().rows.map((row) => (
				<Tr key={row.id}>
					{row.getVisibleCells().map((cell) => (
						<Td
							key={cell.id}
							hidden={cell.column.id === 'hidden' ? true : false}
							className={classNames({
...
const TableTemplate: FC<ITableTemplateProps> = (props) => {
	const { columnas, children, hasHeader, hasFooter, table, ...rest } = props;

	return (
		<Table {...rest}>
			{children || (
				<>
					{hasHeader && <TableHeaderTemplate table={table} columnas={columnas!} />}
